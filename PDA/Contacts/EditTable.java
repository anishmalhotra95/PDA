...
{    
	...
	public EditTable(int sno)
	{
		...
		PreparedStatement psEdit=con.prepareStatement("select rtrim(name),rtrim(address),rtrim(emailid),rtrim(phone),rtrim(dob),rtrim(anndate) from contacts where S_no=?");
		psEdit.setInt(1,sno);
		ResultSet rsEdit=psEdit.executeQuery();
		rsEdit.next();
		fLabel=new Font("arial",Font.BOLD,11);
		fLabelAP=new Font("arial",Font.BOLD,30);
		frame=new JFrame("Edit Record");
		panelmain=new JPanel(new BorderLayout());
		panelAP=new JPanel();
		panelLabel=new JPanel();
		panelButton=new JPanel();

		labelAP=new JLabel("Edit Record");
		...
		textDOB=new JTextField(13);
		textAnnDate=new JTextField(13);
		String strName=rsEdit.getString(1);
		textName.setText(strName);
		textAddress.setText(rsEdit.getString(2));
		textEmailID.setText(rsEdit.getString(3));
		textPhone.setText(rsEdit.getString(4));
		textDOB.setText(rsEdit.getString(5));
		textAnnDate.setText(rsEdit.getString(6));
		...
		if(evt.getSource()==buttonSave)
		{
			try
			{
				...
				else
				{
					...
					psSave=conSave.prepareStatement("update Contacts set name=?,address=?,emailid=?,phone=?,dob=?,anndate=? where s_no=?");	
